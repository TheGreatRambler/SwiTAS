...
// This is used by the switch to determine size, a vector is always send back
// enyway
...
namespace MemoryDataInfo {
	...
	typedef enum {
		...
		MemType_Io = 0x01,     ///< Mapped by kernel capability parsing in \ref
							   ///< svcCreateProcess.
		MemType_Normal = 0x02, ///< Mapped by kernel capability parsing in \ref
							   ///< svcCreateProcess.
		MemType_CodeStatic = 0x03, ///< Mapped during \ref svcCreateProcess.
		MemType_CodeMutable
		= 0x04, ///< Transition from MemType_CodeStatic performed by \ref
		...
		= 0x08, ///< Mapped using \ref svcMapProcessCodeMemory.
		MemType_ModuleCodeMutable
		= 0x09, ///< Transition from \ref MemType_ModuleCodeStatic performed by
				///< \ref svcSetProcessMemoryPermission.
		...
		= 0x0D, ///< Mapped using \ref svcMapTransferMemory when the owning
				///< process has perm=0.
		MemType_TransferMem = 0x0E, ///< Mapped using \ref svcMapTransferMemory
		...
		MemType_KernelStack
		= 0x13, ///< Mapped in kernel during \ref svcCreateThread.
		MemType_CodeReadOnly
		= 0x14, ///< Mapped in kernel during \ref svcControlCodeMemory.
		MemType_CodeWritable
		= 0x15, ///< Mapped in kernel during \ref svcControlCodeMemory.
	} MemoryType;
	...
	typedef enum {
		...
		= BIT(9), ///< Force read/writable by debug syscalls.
		...
		= BIT(14),                     ///< Process permission change allowed.
		MemState_MapAllowed = BIT(15), ///< Map allowed.
		MemState_UnmapProcessCodeMemAllowed
		= BIT(16), ///< Unmap process code memory allowed.
		MemState_TransferMemAllowed = BIT(17), ///< Transfer memory allowed.
		MemState_QueryPAddrAllowed
		= BIT(18), ///< Query physical address allowed.
		MemState_MapDeviceAllowed
		= BIT(19), ///< Map device allowed (\ref svcMapDeviceAddressSpace and
				   ///< \ref svcMapDeviceAddressSpaceByForce).
		MemState_MapDeviceAlignedAllowed
		= BIT(20),                           ///< Map device aligned allowed.
		...
		= MemState_IsPoolAllocated,           ///< Alias for \ref
	...
	typedef enum {
		...
		= BIT(2), ///< Is device mapped (when DeviceRefCount > 0).
		MemAttr_IsUncached = BIT(3), ///< Is uncached.
	} MemoryAttribute;

	static std::unordered_map<MemoryAttribute, std::string>
		memoryAttributeName {
	...
	typedef struct {
		...
		static void serialize(Archive& archive, Self& self) {
			archive(self.addr, self.size, self.type, self.attr, self.perm,
				self.device_refcount, self.ipc_refcount);
		}
	} MemoryInfo;
};
...
namespace Protocol {
	...
	, self.controllerDataPaths, self.extraDataPath)
