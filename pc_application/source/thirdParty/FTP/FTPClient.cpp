...
namespace embeddedmz {
	 ...
	 * @endcode
	 */
	const bool CFTPClient::InitSession(const std::string& strHost, const unsigned& uPort, const std::string& strLogin, const std::string& strPassword, const FTP_PROTOCOL& eFtpProtocol /* = FTP */, const SettingsFlag& eSettingsFlags /* = ALL_FLAGS */) {
		if(strHost.empty()) {
			if(m_eSettingsFlags & ENABLE_LOG)
	...
	const bool CFTPClient::DownloadWildcard(const std::string& strLocalDir, const std::string& strRemoteWildcard) const {
		...
		if(stat(data.strOutputPath.c_str(), &info) == 0 && (info.st_mode & S_IFDIR)) // S_ISDIR() doesn't exist on windows
		{
			...
			if(res != CURLE_OK && res != CURLE_REMOTE_FILE_NOT_FOUND) {
				if(m_eSettingsFlags & ENABLE_LOG)
					m_oLog(StringFormat(LOG_ERROR_CURL_GETWILD_FORMAT, m_strServer.c_str(), strRemoteWildcard.c_str(), res, curl_easy_strerror(res)));
			}
			/* folders need to be copied integrally */
				...
				for(const auto& Dir : data.vecDirList) {
					if(!DownloadWildcard(data.strOutputPath + Dir, strBaseUrl + Dir + "/*")) {
						m_oLog(StringFormat(LOG_ERROR_CURL_GETWILD_REC_FORMAT, (strBaseUrl + Dir + "/*").c_str(), (data.strOutputPath + Dir).c_str()));
						bRet = false;
					}
				}
			} else
