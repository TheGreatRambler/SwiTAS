...
SideUI::SideUI(rapidjson::Document settings, wxFlexGridSizer* sizer, std::shared_ptr<DataProcessing> input) {
	...
	wxBitmap play(wxT(mainSettings["ui"]["playButton"].GetString()), wxBITMAP_TYPE_PNG);
	wxBitmap frameAdvance(wxT(mainSettings["ui"]["frameAdvanceButton"].GetString()), wxBITMAP_TYPE_PNG);
