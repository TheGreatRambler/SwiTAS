...
void HELPERS::readFullFileData(FILE* file, void* bufPtr, int size) {
	...
	while(sizeActuallyRead != size) {
		int bytesRead
			= fread(&buf[sizeActuallyRead], size - sizeActuallyRead, 1, file);
		sizeActuallyRead += bytesRead;
	}
}

void HELPERS::replaceInString(
	std::string input, std::string initial, std::string final) {
	int index;
