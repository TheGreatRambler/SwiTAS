...
namespace loguru {
	...
	template <typename... Args> LOGURU_EXPORT Text textprintf(LOGURU_FORMAT_STRING_TYPE format, const Args&... args) {
...
#undef MAX
	...
	*/
	...
	template <typename... Args> LOGURU_EXPORT void log(Verbosity verbosity, const char* file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, const Args&... args) {
		vlog(verbosity, file, line, format, fmt::make_format_args(args...));
	}

	// Log without any preamble or indentation.
	template <typename... Args> LOGURU_EXPORT void raw_log(Verbosity verbosity, const char* file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, const Args&... args) {
	...
	template <typename... Args> LOGURU_EXPORT LOGURU_NORETURN void log_and_abort(int stack_trace_skip, const char* expr, const char* file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, const Args&... args) {
	...
	*/
	...
	template <unsigned long long N> struct decay_char_array<const char (&)[N]> { using type = const char*; };

	template <class T> struct make_const_ptr { using type = T; };

	template <class T> struct make_const_ptr<T*> { using type = const T*; };

	template <class T> struct make_ec_type { using type = typename make_const_ptr<typename decay_char_array<T>::type>::type; };
...
*/
#define ERROR_CONTEXT(descr, data) const loguru::EcEntryData<loguru::make_ec_type<decltype(data)>::type> LOGURU_ANONYMOUS_VARIABLE(error_context_scope_)(__FILE__, __LINE__, descr, data, static_cast<loguru::EcEntryData<loguru::make_ec_type<decltype(data)>::type>::Printer>(loguru::ec_to_text)) // For better error messages
...
#define DEFINE_CHECK_OP_IMPL(name, op)                                                                                                                                                                                                                                                                                                                                                                         \
	template <typename T1, typename T2> inline std::string* name(const char* expr, const T1& v1, const char* op_str, const T2& v2) {                                                                                                                                                                                                                                                                           \
...
#undef DEFINE_CHECK_OP_IMPL
	...
	template <class T> inline const T& referenceable_value(const T& t) {
		return t;
	}
